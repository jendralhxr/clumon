...
#define DISTANCE 5 // px from marker edge
unsigned int offset;
...
unsigned int calculate_moment(int markernumber, int x_mid, int y_mid){
	...
	for (edge_top= y_mid; edge_top>0; edge_top--){
		//cout << "val:" << edge_top << " " << int (image.data[ edge_top*image_width + x_mid]) << endl;
	...
	for (y= edge_top; y<edge_bottom; y++){
		for (x= edge_left; y<edge_right; x++){
...
int main(int argc, char **argv) {
	...
	transpose(image_input, image);  // two lines, rotate 90 deg clockwise
	flip(image, image, 1);
